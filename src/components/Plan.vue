...
import PlanStats from "./PlanStats.vue"
...
function tabOnClick(tabName : string){
  setActiveTab(tabName)
}

const callBack = (nodeName : number)=>{
  setActiveTab("plan");
  if(nodeName !== undefined && nodeName != selectedNodeId.value){
    // Delayed to make sure the tab has changed before recentering
    setTimeout(() => {
        selectNode(nodeName, true)
      }, 1)
  }
}
...
<template>
  <div
    class="plan-container d-flex flex-column overflow-hidden flex-grow-1 bg-light d-flex flex-column vh-100"
    ref="rootEl"
  >
      ...
      <ul class="nav nav-pills">
        <li class="nav-item p-1">
          <a
            ...
            v-on:click.prevent="tabOnClick('plan')"
            >Plan</a
          >
        </li>
        <li class="nav-item p-1">
          <a
            ...
            v-on:click.prevent="tabOnClick('grid')"
            >Grid
            <span
              class="badge bg-info"
        ...
        <li class="nav-item p-1">
          <a
            ...
            v-on:click.prevent="tabOnClick('raw')"
            >Raw</a
          >
        </li>
        <li class="nav-item p-1">
          <a
            ...
            v-on:click.prevent="tabOnClick('query')"
            >Query</a
          >
        </li>
        <li class="nav-item p-1">
          <a
            ...
            v-on:click.prevent="tabOnClick('stats')"
            >Stats</a
          >
        </li>
      </ul>
    ...
    <div class="tab-content flex-grow-1 d-flex overflow-hidden">
      ...
      >
        <div class="overflow-hidden d-flex w-100 h-100 flex-column">
          ...
          <grid class="flex-grow-1 overflow-auto plan-grid" @node-click="(n)=>callBack(n)"> </grid>
        </div>
      </div>
      <div
        class="tab-pane flex-grow-1 overflow-hidden position-relative"
